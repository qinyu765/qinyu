...
> [!IMPORTANT]
> **架构已迁移**：2026 年 3 月项目已从 Vite SPA (CSR) 迁移至 **Next.js 15 (App Router, SSG)**。本文记录的是迁移前的 SEO 优化历程，其中许多问题（空壳 HTML、HashRouter、CSR 局限）已随 SSG 迁移自动解决。保留为学习参考。
...
> [!NOTE]
> 以下是迁移前的局限分析。项目已迁移至 Next.js SSG，第 1 点已完全解决，第 2、3 点仍然待办。

1. ~~**首屏 HTML 仍然是空的**~~ — ✅ 已通过 SSG 解决，构建时生成完整 HTML
